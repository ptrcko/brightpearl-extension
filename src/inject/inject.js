...
var reportURLPart=   "report.php?results_from=1"
    ...
    +"&submit2=Filter"
    +"&preset-columns=orders_id%2Corders_invoice_num%2Corders_customer_ref%2Corder_allocation_status_id%2Corder_stock_status_id%2Cshipping_status_id%2Corders_status_name%2Corders_tax_date%2Ccustomers_id%2Corder_amount%2Camount_paid%2Cdate_purchased%2Cactions"
    +"&limit=50";
...
//Generate the bpReportURL
bpReportURLPart ="https://" + urlParts[1] + ".brightpearlapp.com/";
...
function init(){
    ...
    // CHANGED for MV3: chrome.extension.getURL(...) → chrome.runtime.getURL(...)
    script.src = chrome.runtime.getURL('src/inject/scripts.js');
    document.head.appendChild(script);
    
    //Insert date script to use on invoice pages
    var script2 = document.createElement('script');
    // CHANGED for MV3: chrome.extension.getURL(...) → chrome.runtime.getURL(...)
    script2.src = chrome.runtime.getURL('src/inject/date-en-AU.js');
    ...
    // CHANGED for MV3: chrome.extension.sendRequest(...) → chrome.runtime.sendMessage(...)
